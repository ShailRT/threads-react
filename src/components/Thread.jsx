...
function Thread({ user, filteredThread, setOpenPopUp, getThreads, setInteratingThreads }) {
  const timePassed = moment().startOf("day").fromNow(filteredThread.timestamp);
  const [replyLength, setReplyLength] = useState(0)
  const handleClick = () => {
    setOpenPopUp(true);
    setInteratingThreads(filteredThread)
  };

  console.log(filteredThread, "filteredThreads");
